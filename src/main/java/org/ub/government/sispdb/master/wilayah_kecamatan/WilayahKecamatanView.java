package org.ub.government.sispdb.master.wilayah_kecamatan;
...
import org.ub.government.sispdb.model_table.TableModel_WilayahKecamatan;
...
public class WilayahKecamatanView extends FormTemplate1_IntFrame{
	protected WilayahKecamatanController controller;
	protected WilayahKecamatanModel model;
	
	private TableRowSorter<TableModel_WilayahKecamatan> tableRowSorter;
	 ...
	 */
		...
		public WilayahKecamatanView() {
			super();
			controller = new WilayahKecamatanController(this);
...
//			setReloadComponentGrid2(); // Tidak Perlu di reload dulu
		...
		public void initComponentView(){
			this.setTitle("MASTER KECAMATAN");
...
//			getjScrollPane1().setVisible(false);
			...
			getLabelGroup1().setText("KAB/KOTA");
			getLabelGroup2().setText("KAB/KOTA");
...
//			dataProvider.refreshAll();
			if (getTf_Filter1().getText().isEmpty()) {
				model.tableModelHeader = new TableModel_WilayahKecamatan(new ArrayList<>(model.listHeader.values()) );
			}else {
				model.tableModelHeader = new TableModel_WilayahKecamatan(new ArrayList<>(model.listHeader.values()) );
			}
		...
		public void reloadComponentComboBox(){
			//Change To Array
			Kabupaten[] itemsArray = new Kabupaten[model.listGrup1.size()];
			itemsArray = model.listGrup1.toArray(itemsArray);
			
			getCombo_Group1().setModel(new DefaultComboBoxModel<Kabupaten>(itemsArray));
...
//			grid1.setDataProvider(dataProvider);
			...
			tableRowSorter = new TableRowSorter<TableModel_WilayahKecamatan>(model.tableModelHeader);
			getjTable1().setRowSorter(tableRowSorter);
			
		}
		
		public void setTableModelOrFilter() {
			RowFilter<TableModel_WilayahKecamatan,Object> rowFilter = null;
			
			ArrayList<RowFilter<TableModel_WilayahKecamatan, Object>> arrListFilters = new ArrayList<RowFilter<TableModel_WilayahKecamatan,Object>>();
			RowFilter<TableModel_WilayahKecamatan,Object> kode1_Filter = null;
			RowFilter<TableModel_WilayahKecamatan,Object> desc_Filter = null;
		...
		public void setTableModelFilter() {
		    RowFilter<TableModel_WilayahKecamatan, Object> rf = null;
...
//            getTa_Notes().setText(model.itemHeader.getNotes());
            ...
            getCombo_Group1().setSelectedItem( model.itemHeader.getKabupatenBean()); 
	    }
...
//			model.itemHeader.setNotes(getTa_Notes().getText().trim());
			...
			model.itemHeader.setKabupatenBean((Kabupaten) getCombo_Group1().getSelectedItem()); 
		}
...
class ComboBoxGroup1Renderer extends JLabel implements ListCellRenderer<Kabupaten> {
	public ComboBoxGroup1Renderer() {
        setOpaque(true);
    }
    @Override
    public Component getListCellRendererComponent(JList<? extends Kabupaten> list, Kabupaten domain, int index,
