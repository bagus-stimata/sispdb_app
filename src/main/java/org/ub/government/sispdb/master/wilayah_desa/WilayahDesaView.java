package org.ub.government.sispdb.master.wilayah_desa;
...
import org.ub.government.sispdb.model_table.TableModel_WilayahDesa;
...
public class WilayahDesaView extends FormTemplate1_IntFrame{
	protected WilayahDesaController controller;
	protected WilayahDesaModel model;
	
	private TableRowSorter<TableModel_WilayahDesa> tableRowSorter;
	 ...
	 */
		...
		public WilayahDesaView() {
			super();
			controller = new WilayahDesaController(this);
...
//			dataProvider.refreshAll();
			if (getTf_Filter1().getText().isEmpty()) {
				model.tableModelHeader = new TableModel_WilayahDesa(new ArrayList<>(model.listHeader.values()) );
			}else {
				model.tableModelHeader = new TableModel_WilayahDesa(new ArrayList<>(model.listHeader.values()) );
			}
		...
		public void reloadComponentComboBox(){
			//Change To Array
			Kecamatan[] itemsArray = new Kecamatan[model.listGrup1.size()];
...
//			grid1.setDataProvider(dataProvider);
			...
			tableRowSorter = new TableRowSorter<TableModel_WilayahDesa>(model.tableModelHeader);
			getjTable1().setRowSorter(tableRowSorter);
			
		}
		
		public void setTableModelOrFilter() {
			RowFilter<TableModel_WilayahDesa,Object> rowFilter = null;
			
			ArrayList<RowFilter<TableModel_WilayahDesa, Object>> arrListFilters = new ArrayList<RowFilter<TableModel_WilayahDesa,Object>>();
			RowFilter<TableModel_WilayahDesa,Object> kode1_Filter = null;
			RowFilter<TableModel_WilayahDesa,Object> desc_Filter = null;
		...
		public void setTableModelFilter() {
		    RowFilter<TableModel_WilayahDesa, Object> rf = null;
...
//            getTa_Notes().setText(model.itemHeader.getNotes());
            ...
            getCombo_Group1().setSelectedItem( model.itemHeader.getKecamatanBean() ); 
	    }
...
//			model.itemHeader.setNotes(getTa_Notes().getText().trim());
			...
			model.itemHeader.setKecamatanBean((Kecamatan) getCombo_Group1().getSelectedItem()); 
		}
...
class ComboBoxGroup1Renderer extends JLabel implements ListCellRenderer<Kecamatan> {
	public ComboBoxGroup1Renderer() {
        setOpaque(true);
    }
    @Override
    public Component getListCellRendererComponent(JList<? extends Kecamatan> list, Kecamatan domain, int index,
